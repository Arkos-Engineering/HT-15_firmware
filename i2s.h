/* i2s.h
 *
 * Original Author: Daniel Collins
 * Date:   2022-02-25
 *
...
typedef struct pio_i2s {
    ...
    u32 dma_ch_out_ctrl;
    u32 dma_ch_out_data;
    i32* in_ctrl_blocks[2];  // Control blocks MUST have 8-byte alignment.
    i32* out_ctrl_blocks[2];
    i32 input_buffer[STEREO_BUFFER_SIZE * 2];
    i32 output_buffer[STEREO_BUFFER_SIZE * 2];
    i2s_config config;
} pio_i2s;

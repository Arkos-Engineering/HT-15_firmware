...
float rfmodule_2m70cm_set_symbol_rate_sps(rfmodule_2m70cm_state_t *dev, float rate_sps);
...
/*
* sets the symbol rate to rate_sps, and returns the exact rate set
*/
float rfmodule_2m70cm_set_symbol_rate_sps(rfmodule_2m70cm_state_t *dev, float rate_sps){

	// Datasheet says that the cc1200 works in ksps, but testing with SmartRF studio
	// shows that it actually is sps. - cc1200-driver.cpp on github

    // srate_e is a 4 bit uint, srate_m is a 20 bit uint
    // e=0: rate_sps = (srate_m/2^38)*F_XOSC
    // e>0: rate_sps = (((2^20+srate_m)*2^srate_e)/2^39)*F_XOSC
    u32 srate_m = 0;
    u8 srate_e = 0;
    ...
    if(rate_sps <= max_symbol_rate_if_srate_e_is_zero){
        srate_m = (u32)((rate_sps / xosc_float) * (double)(1ULL << 38)+0.5);
    ...
    // printf("rate_mantissa: %i, rate_exponent: %i\n", srate_m, srate_e);
    ...
    if(srate_e == 0){
        actual_rate_sps = ((double)srate_m / (double)(1ULL << 38)) * xosc_float;
    } else {
        actual_rate_sps =
    ...
    dev->current_symbol_rate = actual_rate_sps;
    return actual_rate_sps;
}
...
u8 rfmodule_2m70cm_set_upsampler(rfmodule_2m70cm_state_t *dev, u8 factor){
    ...
    while (factor > 1 &&
        ((u64)dev->current_symbol_rate * (u64)16 * (u64)factor) > ((u64)F_XOSC / ((u64)4u))) {
        factor >>= 1;
    }
